...
public class HashBoard extends Board {
  ...
  public Board copy()
  {
    HashMap<TPos,Tile> copy = (HashMap<TPos,Tile>) this.tiles.clone();
    HashBoard newBoard = new HashBoard();
    newBoard.tiles = copy;
    newBoard.rows = this.rows;
    newBoard.cols = this.cols;
    newBoard.lastShiftMovedTiles = this.lastShiftMovedTiles;
    return newBoard;
  }
  ...
  {
  	  //are they hashboards?
    if(!(o instanceof HashBoard)){
      return false;
    }
    //o is converted to a HashBoard for convenience
    HashBoard hb = (HashBoard) o;
    
    //Do they have the same dimensions?
    if(this.rows!=hb.rows)
    	return false;
    if(this.cols!=hb.cols)
    	return false;
    
    //an iterator is set up to compare tiles
    Set<TPos> setOfKeys = this.tiles.keySet();
  	Iterator<TPos> iter = setOfKeys.iterator();
  	
  	//the while loop moves from tile to tile in the board
  	//	and compares each position
    while(iter.hasNext())
    {
    	TPos tilePos = iter.next();
    	int i = tilePos.row;
    	int j = tilePos.col;
    	Tile t = hb.tileAt(i,j);
    	
    	//do they have the same tile at the same position?
    	//tile and tile
    	if((tilePos!=null)&&(t!=null))
    	{
    		if(!t.equals(this.tiles.get(tilePos)))
    			return false;
    	}
    	//tile and null
    	if((tilePos==null)&&(t!=null))
    		return false;
    	//null and tile
    	if((t==null)&&(tilePos!=null))
    		return false;
    	//if both are null, nothing is done because they match.
    }
    return true;
  }
      
  //Computes a fair hash code for the board.  Use a
  // polynomial like code. Accounts for the size and contents of the
  // board. 
  public int hashCode()
  {
  	  //hashCo will compile the board's overall hashCode
  	  int hashCo = 0;
  	  
  	  //for loops move through each position on the board and create a key
  	  //	out of the current position to cll on the hashtable with.
  	  for(int r = 0; r<rows; r++)
  	  {
  	  	  for(int c = 0; c<cols; c++)
  	  	  {
  	  	  	  //the current position is made into a key and used to call
  	  	  	  //	th tile in that position.
  	  	  	  TPos location = new TPos(r, c);
  	  	  	  int locationHash = location.hashCode();
  	  	  	  Tile currentTile = this.tiles.get(location);
  	  	  	  
  	  	  	  //if no tile is returned, the location's hashcode is simply 
  	  	  	  //	added to the overall hashCode
  	  	  	  if(currentTile==null)
  	  	  	  {
  	  	  	  	  hashCo += locationHash;
  	  	  	  }
  	  	  	  //if a tile is returned, step1 converts it into a string and 
  	  	  	  //	step2 converts it into a character array. Each character
  	  	  	  //	is converted into an int and the sum of the character values
  	  	  	  // 	is multiplied by the tile's location before being added
  	  	  	  //	to the overall hashCode.
  	  	  	  else
  	  	  	  {
  	  	  	  	  int tileHash = currentTile.hashCode();
  	  	  	  	  hashCo += tileHash+(tileHash*locationHash);
  	  	  	  }
  	  	  }
  	  }
  	  // the hashCode for the whole hashBoard is returned.
  	  return hashCo;
  }
}
