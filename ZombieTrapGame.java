...
public class ZombieTrapGame{
  ...
  // Makes an exact copy of the game
  public ZombieTrapGame copy(){
    ZombieTrapGame newGame = new ZombieTrapGame();
    newGame.board = this.board.copy();
    newGame.score = this.score;
    newGame.random = this.random;
    return newGame;
  }

  // Checks to see if o is 
  public boolean equals(Object o){
    if(!(o instanceof ZombieTrapGame)){
      return false;
    }
    //o is converted to a ZombieTrapGame for convenience
    ZombieTrapGame copy = (ZombieTrapGame) o;
    HashBoard hb = (HashBoard) this.board;
    HashBoard other = (HashBoard) copy.board;
    //do the boards have the same set-up?
    if(!hb.equals(other))
    	return false;
    	
    return copy.getScore()==this.score;
  }

  // The hash code should be based on the state the board and the 
  //current score of the game.  Two games with the same same tiles but a 
  //different score should have different hash codes.
  public int hashCode(){
    HashBoard hb = (HashBoard) this.board;
    int hashCo = hb.hashCode();
    return this.score+hashCo;
  }
